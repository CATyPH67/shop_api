...
class CartService:
    ...
    async def add_to_cart(self, user, payload: CartItem) -> CartOut:
        ...
        cart = await self.repo.get_cart(user.id)
        if not cart:
            cart = await self.repo.create_cart(user.id)
            logger.info("New cart created", extra={"extra_fields": {"user_id": user.id, "cart_id": cart.id}})

        items = await self.repo.get_items(cart.id)
        existing_item = await self.repo.get_item_by_product(cart.id, payload.product_id)
    ...
    async def update_cart_item(self, item_id: int, user, payload: CartItemFields) -> CartOut:
        ...
        cart_item = await self.repo.get_item_by_id(cart.id, item_id)
        if not cart_item:
        ...
        if payload.quantity == 0:
            ...
            logger.info(
                "Cart item updated",
                extra={"extra_fields": {"cart_id": cart.id, "item_id": item_id, "product_id": payload.product_id, "quantity": payload.quantity}}
            )
